...
if (-not $OutputRoot) { $OutputRoot = Join-Path $repository 'artifacts/imports' }
...
$record = [ordered]@{ schema_version = 1; task_id = 'P10-02'; status = 'running'; started_at = [DateTime]::UtcNow.ToString('o'); stages = @() }
...
function Stage([string]$Name, [string[]]$Arguments, [int]$Expected) {
    ...
    Write-Host "Import qualification: $Name"
    & cargo @Arguments *> $log
    $code = $LASTEXITCODE
...
try {
    ...
    $inputs = @('scripts/test-imports.ps1', 'src/third_party/codex/codex-rs/Cargo.lock', 'src/third_party/components/codex-files.json', 'src/crates/vcp-extensions/tests/fixtures/imports/codex.toml', 'src/crates/vcp-extensions/tests/fixtures/imports/gemini.json')
    ...
    try {
        ...
        Stage 'pure-imports' ($base + @('-p', 'vcp-extensions', '--test', 'imports')) 7
        Stage 'cli-contracts' ($base + @('-p', 'vcp-cli', '--features', 'qualification', '--lib', '--', '--test-threads=1')) 100
        ...
        Stage 'native-imports' ($base + @('-p', 'vcp-cli', '--features', 'qualification', '--test', 'config_import', '--', '--test-threads=1')) 5
        Stage 'cli-executable' ($base + @('-p', 'vcp-cli', '--features', 'qualification', '--test', 'executable', '--', '--test-threads=1')) 29
