...
try{
    ...
    try{
        ...
        Stage 'connected' ($common+$hostTests+@('--','--test-threads=1')) @('child_native_write_process_kill_retains_unknown_effect_without_replay','graph_child_dispatch_uses_isolated_bytes_root_budget_and_parent_pause_gate','child_result_integration_uses_parent_broker_and_rejects_concurrent_parent_changes','child_fixed_model_mismatch_never_sends_or_reserves_budget','fresh_owner_recovers_registered_child_edits_and_keeps_sibling_paused','child_integration_partial_native_failure_retains_receipts_and_human_edits','child_integration_pause_between_writes_retains_receipts_and_human_edits','child_integration_pause_and_cancel_before_and_after_each_write','child_integration_process_kill_after_each_write_reopens_without_replay','integrated_parent_verification_pause_and_cancel_before_publish','child_near_budget_race_admits_one_wire_request_and_preserves_shared_root','child_sibling_disjoint_and_overlapping_writes_remain_isolated_and_scope_bound','child_dependency_blocks_launch_until_current_analysis_verification_completes','child_individual_cancellation_preserves_calling_sibling_and_exact_liability','child_unexpected_provider_disconnect_still_pauses_root_with_full_liability','child_late_result_after_parent_steering_retains_evidence_without_integration','child_cleanup_retains_durable_intent_and_results_and_reconciles_only_original_root','helper_templates_admit_current_read_only_assignments_and_fence_parent_pause')
        ...
        Stage 'child-output-loss' ($common+@('-p','vcp-cli','--features','qualification','--test','executable','noisy_and_quiet_registered_children_survive_output_loss_and_cursor_recovery','--','--test-threads=1')) @('noisy_and_quiet_registered_children_survive_output_loss_and_cursor_recovery')
