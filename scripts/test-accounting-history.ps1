...
if(-not $OutputRoot){$OutputRoot=Join-Path $repository 'artifacts/accounting-history'}
...
function Stage([string]$Name,[string]$Program,[string[]]$Arguments){
    $log=Join-Path $directory ($Name+'.log');Write-Host "Accounting/history qualification: $Name"
...
try{
    ...
    $packages=@('vcp-domain','vcp-protocol','vcp-store','vcp-engine','vcp-budget','vcp-audit')
    ...
    try{
        ...
        if($rows.Count -ne 39){throw "Expected all 39 foundation/accounting/history contracts; observed $($rows.Count)"}
