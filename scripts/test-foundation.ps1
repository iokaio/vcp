...
if(-not $OutputRoot){$OutputRoot=Join-Path $repository 'artifacts/foundation'}
...
$record=[ordered]@{schema_version=1;task_id='P1-01/P1-02/P1-03/P1-04';status='prepared';started_at=[DateTime]::UtcNow.ToString('o');stages=@()}
...
function Stage([string]$Name,[string]$Program,[string[]]$Arguments){
    $log=Join-Path $directory ($Name+'.log');Write-Host "Foundation qualification: $Name"
...
try{
    ...
    $packages=@('vcp-domain','vcp-protocol','vcp-store','vcp-engine')
    ...
    try{
        $arguments=@('+1.98.0','test','--locked','--offline','--target','x86_64-pc-windows-msvc','-j',"$Jobs",'--features','vcp-store/qualification')
        ...
        if($rows.Count -ne 24){throw "Expected all 24 foundation contracts; observed $($rows.Count)"}
