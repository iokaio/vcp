...
if (-not $OutputRoot) { $OutputRoot = Join-Path $repository 'artifacts/p7-mcp-stdio-qualification' }
...
$record = [ordered]@{
    schema = 'p7-mcp-stdio-qualification/1'; task = 'P7-03'; status = 'running'
    ...
    scope = 'Bounded stdio MCP client, fixture and canonical host; no HTTP, resources or credential qualification'
}
...
try {
    ...
    Stage 'mcp-core' ($common + @('-p', 'vcp-extensions', '--lib', '--test', 'mcp_client', '--test', 'mcp_identity', '--test', 'mcp_schema', '--', '--test-threads=1'))
    ...
    Stage 'lifecycle-unit-regression' ($common + @('-p', 'vcp-lifecycle', '--features', 'qualification', '--lib', '--', '--test-threads=1'))
