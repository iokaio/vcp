...
if (-not $OutputRoot) { $OutputRoot = Join-Path $repository 'artifacts/p7-builtin-skill-qualification' }
...
$record = [ordered]@{
    schema = 'p7-builtin-skill-qualification-run/1'; started_at = [DateTime]::UtcNow.ToString('o')
    status = 'running'; stages = @(); report = 'builtin.json'
...
function Source-Identity {
    $identity = & node (Join-Path $PSScriptRoot 'builtin-source-identity.cjs')
...
try {
    ...
    $common = @('--manifest-path', (Join-Path $repository 'src/third_party/codex/codex-rs/Cargo.toml'), '--locked', '--offline', '--target-dir', ([IO.Path]::GetFullPath($TargetRoot)), '-j', "$Jobs", '-p', 'vcp-lifecycle', '--example', 'builtin_skill_qualification')
    ...
    Stage 'comparison' (@('run') + $common + @('--', (Join-Path $directory 'builtin.json')))
    $record.report_sha256 = (Get-FileHash -LiteralPath (Join-Path $directory 'builtin.json') -Algorithm SHA256).Hash.ToLowerInvariant()
