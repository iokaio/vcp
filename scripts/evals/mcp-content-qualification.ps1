...
if (-not $OutputRoot) { $OutputRoot = Join-Path $repository 'artifacts/p7-mcp-content-qualification' }
...
$record = [ordered]@{
    schema = 'p7-mcp-content-qualification/1'; task = 'P7-03'; status = 'running'
    ...
    scope = 'Canonical MCP resources and prompts, scoped artifact cache, external content and stdio/HTTPS regression'
}
...
try {
    ...
    Stage 'mcp-schema-feature-unification' ($common + @('-p', 'vcp-extensions', '--features', 'serde_json/arbitrary_precision', '--test', 'mcp_schema', '--', '--test-threads=1'))
