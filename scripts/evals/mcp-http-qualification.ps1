...
if (-not $OutputRoot) { $OutputRoot = Join-Path $repository 'artifacts/p7-mcp-http-qualification' }
...
$record = [ordered]@{
    schema = 'p7-mcp-http-qualification/1'; task = 'P7-03'; status = 'running'
    ...
    scope = 'Canonical remote MCP tools, bounded streaming, scoped credentials, native trust and physical socket fences'
}
...
try {
    ...
    Stage 'mcp-core' ($common + @('-p', 'vcp-extensions', '--lib', '--test', 'mcp_client', '--test', 'mcp_identity', '--test', 'mcp_schema', '--test', 'mcp_http', '--test', 'mcp_http_session', '--', '--test-threads=1'))
