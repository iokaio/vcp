...
fn main() -> ExitCode {
    match run() {
        ...
        Err(error) => {
            eprintln!("CR06 check launcher: {error}");
            ExitCode::FAILURE
        }
    }
}
...
fn run() -> Result<u8, Box<dyn std::error::Error>> {
    ...
    if args.len() != expected.len() || args.iter().zip(expected).any(|(a, b)| a != b) {
        return Err("only the frozen CR06 verification command is permitted".into());
    }
    ...
    let status = Command::new(env!("VCP_CR06_NODE"))
        .args([
            "--permission",
            "--test-isolation=none",
        ...
        .args(args)
        .env_clear()
        .env("SystemRoot", env!("VCP_CR06_SYSTEMROOT"))
        .stdin(Stdio::null())
