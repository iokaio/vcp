...
const redactOutput = value => JSON.parse(JSON.stringify(value).replaceAll(/vcp-authoring-candidates::\.::(?:document-authoring|skill-authoring)|vcp-builtin::(?:architecture::architecture|testing::testing)/g, '[selected skill]'));
...
function inspect(envelopeFile, envelopeHash, candidate, phase) {
  const runner = require('./authoring-qualification.cjs'), envelope = runner.envelopeFor(envelopeFile, envelopeHash);
...
function packets(envelopeFile, envelopeHash, candidate, phase, destination) {
  ...
  const forbidden = [path.resolve(__dirname, '../..'), envelope.directory, envelope.budget.current.reference.repository, path.dirname(envelope.budget.current.reference.plan.file), ...envelope.budget.history.phases.map(item => path.dirname(item.reference.envelope.file))].filter(Boolean);
...
function project(directory, phaseHash, resultHash, rawBytes, sourceRef) {
  ...
  if (!object(commit, ['schema', 'phase_sha256', 'result_sha256', 'packets', 'mapping_sha256']) || commit.schema !== 'cs1-qualification-blind-commitment/1' || commit.phase_sha256 !== phaseHash || commit.result_sha256 !== resultHash) throw Error('Blind commitment names another phase/result');
  ...
  if (!object(map, ['schema', 'salt', 'phase_sha256', 'result_sha256', 'mappings']) || !/^[a-f0-9]{64}$/.test(map.salt) || map.schema !== 'cs1-qualification-private-mapping/1' || map.phase_sha256 !== phaseHash || map.result_sha256 !== resultHash || !Array.isArray(map.mappings) || !Array.isArray(packet.cases) || !object(raw, ['schema', 'reviewer_id', 'independent_blinded', 'packets_sha256', 'cases']) || raw.schema !== 'cs1-qualification-blind-review/1' || raw.independent_blinded !== true || raw.packets_sha256 !== commit.packets.sha256 || typeof raw.reviewer_id !== 'string' || !raw.reviewer_id.trim() || !Array.isArray(raw.cases) || !equal(raw.cases.map(item => item.case_id).sort(), map.mappings.map(item => item.case_id).sort()) || !equal(packet.cases.map(item => item.case_id).sort(), map.mappings.map(item => item.case_id).sort())) throw Error('Independent raw reader coverage or binding differs');
