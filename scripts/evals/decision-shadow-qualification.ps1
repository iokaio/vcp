...
if (-not $OutputRoot) { $OutputRoot = Join-Path $repository 'artifacts/p6-decision-shadow-qualification' }
...
$record = [ordered]@{
    schema = 'p6-decision-shadow-qualification/1'; task = 'P6-02'; status = 'running'
    ...
    scope = 'Canonical routing shadow admission, transport, accounting and cancellation'
}
...
try {
    ...
    Stage 'context-provenance' ($common + @('-p', 'vcp-context', '--', '--test-threads=1'))
    Stage 'decision-codecs' ($common + @('-p', 'vcp-models', '--test', 'decision', '--', '--test-threads=1'))
    Stage 'host-and-transport-unit' ($common + @('-p', 'vcp-lifecycle', '--features', 'qualification', '--lib', '--', '--test-threads=1'))
    Stage 'decision-shadow-host' ($common + @('-p', 'vcp-lifecycle', '--features', 'qualification', '--test', 'canonical_host', 'decision_shadow', '--', '--test-threads=1'))
    Stage 'routing-host' ($common + @('-p', 'vcp-lifecycle', '--features', 'qualification', '--test', 'canonical_host', 'routing::', '--', '--test-threads=1'))
    Stage 'mcp-http-host' ($common + @('-p', 'vcp-lifecycle', '--features', 'qualification', '--test', 'canonical_host', 'mcp_http', '--', '--test-threads=1'))
    Stage 'mcp-final-fault-host' ($common + @('-p', 'vcp-lifecycle', '--features', 'qualification', '--test', 'canonical_host', 'mcp_final_faults', '--', '--test-threads=1'))
    Stage 'cli' ($common + @('-p', 'vcp-cli', '--features', 'qualification', '--lib', '--', '--test-threads=1'))
    $hostLog = Get-Content -LiteralPath (Join-Path $directory 'decision-shadow-host.log') -Raw
    $hostBinaryMatch = [regex]::Match($hostLog, '(?m)^\s*Running .*canonical_host\.rs \((.+\.exe)\)\r?$')
    if (-not $hostBinaryMatch.Success) { throw 'Canonical shadow test executable identity unavailable' }
    ...
    $record.canonical_shadow_fixture_binary_sha256 = (Get-FileHash -LiteralPath $hostBinary -Algorithm SHA256).Hash.ToLowerInvariant()
