...
if (-not $OutputRoot) { $OutputRoot = Join-Path $repository 'artifacts/p7-mcp-http-boundaries-qualification' }
...
$record = [ordered]@{
    schema = 'p7-mcp-http-boundaries-qualification/1'; task = 'P7-03'; status = 'running'
    ...
    scope = 'Bounded HTTP framing, scoped credential leases and physical socket write fences; canonical remote MCP dispatch remains separate'
}
...
try {
    ...
    Stage 'mcp-core' ($common + @('-p', 'vcp-extensions', '--lib', '--test', 'mcp_client', '--test', 'mcp_identity', '--test', 'mcp_schema', '--test', 'mcp_http', '--', '--test-threads=1'))
