...
if (-not $OutputRoot) { $OutputRoot = Join-Path $repository 'artifacts/p5-08-comparison' }
...
$record = [ordered]@{
    schema = 'p5-08-comparison-run/1'; started_at = [DateTime]::UtcNow.ToString('o')
    status = 'running'; stages = @(); report = 'comparison.json'
...
function Source-Identity {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Comparison source identity capture failed' }
...
try {
    ...
    $common = @('--manifest-path', (Join-Path $repository 'src/third_party/codex/codex-rs/Cargo.toml'), '--locked', '--offline', '--target-dir', ([IO.Path]::GetFullPath($TargetRoot)), '-j', "$Jobs", '-p', 'vcp-memory', '--example', 'memory_comparison')
    Stage 'grader' (@('test') + $common)
    Stage 'comparison' (@('run') + $common + @('--', (Join-Path $directory 'comparison.json')))
    $record.report_sha256 = (Get-FileHash -LiteralPath (Join-Path $directory 'comparison.json') -Algorithm SHA256).Hash.ToLowerInvariant()
