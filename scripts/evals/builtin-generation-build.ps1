...
$taskSource = (Resolve-Path (Join-Path $PSScriptRoot 'builtin-generation-launcher.rs')).Path
...
$taskBuildRoot = Join-Path $taskRepository ('artifacts/p7-u03-launcher-' + [guid]::NewGuid().ToString('N'))
$taskLauncher = Join-Path $taskBuildRoot 'u03-check.exe'
...
$taskArguments = @('--edition=2021','--crate-name','vcp_u03_launcher',$taskSource,'-o',$taskLauncher)
$taskPreviousNode = $env:VCP_U03_NODE
$taskPreviousSystemRoot = $env:VCP_U03_SYSTEMROOT
try {
    $env:VCP_U03_NODE = $taskNode
    $env:VCP_U03_SYSTEMROOT = $env:SystemRoot
    ...
    $env:VCP_U03_NODE = $taskPreviousNode
    $env:VCP_U03_SYSTEMROOT = $taskPreviousSystemRoot
}
...
$taskReceipt = [ordered]@{
    schema = 'p7-u03-launcher-build/1'
    ...
    embedded = @{VCP_U03_NODE=$taskNode; VCP_U03_SYSTEMROOT=$env:SystemRoot}
