...
if (-not $OutputRoot) { $OutputRoot = Join-Path $repository 'artifacts/p7-skill-discovery-qualification' }
...
$record = [ordered]@{
    schema = 'p7-skill-discovery-qualification-run/1'; started_at = [DateTime]::UtcNow.ToString('o')
    status = 'running'; stages = @(); report = 'skills.json'
...
function Source-Identity {
    $identity = & node (Join-Path $PSScriptRoot 'skills-source-identity.cjs')
    if ($LASTEXITCODE -ne 0) { throw 'Skills source identity capture failed' }
...
try {
    ...
    $common = @('--manifest-path', (Join-Path $repository 'src/third_party/codex/codex-rs/Cargo.toml'), '--locked', '--offline', '--target-dir', ([IO.Path]::GetFullPath($TargetRoot)), '-j', "$Jobs", '-p', 'vcp-extensions', '--example', 'skill_discovery_qualification')
    Stage 'contracts' @('test', '--manifest-path', (Join-Path $repository 'src/third_party/codex/codex-rs/Cargo.toml'), '--locked', '--offline', '--target-dir', ([IO.Path]::GetFullPath($TargetRoot)), '-j', "$Jobs", '-p', 'vcp-extensions', '--tests')
    Stage 'comparison' (@('run') + $common + @('--', (Join-Path $directory 'skills.json')))
    $record.report_sha256 = (Get-FileHash -LiteralPath (Join-Path $directory 'skills.json') -Algorithm SHA256).Hash.ToLowerInvariant()
