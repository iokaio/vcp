...
try{
    ...
    try{
        ...
        foreach($count in @(5,3,12,16)){if($tests -notmatch "test result: ok\. $count passed; 0 failed; 0 ignored;"){throw "Missing expected $count-test contract group"}}
        $rows=[regex]::Matches($tests,'(?m)^test ([^\r\n]+) \.\.\. ok\r?$')
        if($rows.Count -ne 39){throw 'Expected all 39 native host/port/canonical contracts'}
