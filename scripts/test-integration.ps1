...
try{
    ...
    $inputs+=@(@('vcp-domain','vcp-protocol','vcp-store','vcp-engine','vcp-budget','vcp-audit','vcp-models','vcp-context','vcp-repository','vcp-policy','vcp-tools','vcp-memory')|ForEach-Object{Get-ChildItem -LiteralPath (Join-Path $repository "src/crates/$_") -Recurse -File|Where-Object {$_.Extension -eq '.rs' -or $_.Name -eq 'Cargo.toml'}|ForEach-Object FullName})
