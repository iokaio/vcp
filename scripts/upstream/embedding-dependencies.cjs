...
const { embeddingClosure, verifyReference } = require('../../src/tests/support/dependency-closure.cjs');
...
try {
  ...
  const actual = embeddingClosure(fs.readFileSync(process.argv[2], 'utf8'));
  const reference = JSON.parse(fs.readFileSync(path.join(repository, 'src/third_party/components/embedding-dependencies.json')));
...
} catch (error) { console.error('Embedding dependencies: ' + error.message); process.exitCode = 1; }
