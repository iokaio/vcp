...
if (-not $OutputRoot) { $OutputRoot = Join-Path $repository 'artifacts/observers' }
...
$record = [ordered]@{ schema_version = 1; task_id = 'P10-03'; status = 'running'; started_at = [DateTime]::UtcNow.ToString('o'); stages = @() }
...
function Stage([string]$Name, [string[]]$Arguments, [int]$Expected) {
    ...
    Write-Host "Observer qualification: $Name"
    & cargo @Arguments *> $log
    $code = $LASTEXITCODE
    $passed = 0
    foreach ($summary in [regex]::Matches((Get-Content -LiteralPath $log -Raw), '(?m)^test result: ok\. (\d+) passed;')) {
...
try {
    ...
    $inputs = @('scripts/test-observers.ps1', 'docs/evaluations/p10-03-observer-grading.md', 'src/third_party/codex/codex-rs/Cargo.lock', 'src/third_party/codex/codex-rs/core/src/client.rs', 'src/third_party/codex/codex-rs/ext/extension-api/src/work_admission.rs')
    foreach ($crate in @('vcp-domain', 'vcp-store', 'vcp-memory', 'vcp-engine', 'vcp-lifecycle', 'vcp-cli')) {
    ...
    $record.durability_scope = 'Local bounded observation and canonical recovery; no external observer inference or machine power-loss claim'
    ...
    try {
        ...
        Stage 'pure-observers' ($base + @('-p', 'vcp-engine', '--lib', 'observers::subscription::tests')) 8
        Stage 'observer-retention' ($base + @('-p', 'vcp-memory', '--test', 'retention_observers')) 2
        Stage 'redaction-regressions' ($base + @('-p', 'vcp-store', '--test', 'redacted_receipts', '--test', 'redacted_advisory')) 2
        Stage 'source-cost-limits' ($base + @('-p', 'vcp-lifecycle', '--features', 'qualification', '--lib', 'source_byte_preflight')) 2
        Stage 'diagnostic-matched-campaign' ($base + @('-p', 'vcp-lifecycle', '--features', 'qualification', '--lib', 'observer_diagnostic_matched_campaign', '--', '--nocapture')) 1
        Stage 'cli-contracts' ($base + @('-p', 'vcp-cli', '--features', 'qualification', '--lib', '--', '--test-threads=1')) 101
        Stage 'cli-observers' ($base + @('-p', 'vcp-cli', '--features', 'qualification', '--test', 'executable', 'observers::', '--', '--test-threads=1')) 1
        Stage 'native-observers' ($base + @('-p', 'vcp-lifecycle', '--features', 'qualification', '--test', 'canonical_host', 'observers::', '--', '--test-threads=1', '--nocapture')) 10
        Stage 'verification-regressions' ($base + @('-p', 'vcp-lifecycle', '--features', 'qualification', '--test', 'canonical_host', 'verification::', '--', '--test-threads=1')) 6
