...
if(-not $OutputRoot){$OutputRoot=Join-Path $repository 'artifacts/provider'}
...
$record=[ordered]@{schema_version=1;task_id='P2-01/P2-02';status='prepared';started_at=[DateTime]::UtcNow.ToString('o');stages=@()}
...
function Stage([string]$Name,[string]$Program,[string[]]$Arguments){
    $log=Join-Path $directory ($Name+'.log');Write-Host "Provider qualification: $Name"
...
try{
    ...
    $record.scope='Provider codec and context contracts plus retained absolute deadline; native host integration has its own runner'
    $record.vcp_commit=& git -C $repository rev-parse HEAD
    $packages=@('vcp-models','vcp-context')
    ...
    $inputs+=@(Get-ChildItem -LiteralPath (Join-Path $repository 'src/crates/vcp-repository') -Recurse -File|Where-Object{$_.Extension -eq '.rs' -or $_.Name -eq 'Cargo.toml'}|ForEach-Object FullName)
    ...
    try{
        ...
        if($rows.Count -ne 16){throw "Expected all 16 provider/context contracts; observed $($rows.Count)"}
