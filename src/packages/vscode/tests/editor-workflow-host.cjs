...
exports.run=async()=>{
  ...
  try{
    ...
    phase='stale preview';a=await open(path.join(input.workspace,'value.txt'));await draft(a,'43\n');const stale=await prepare();await vscode.window.showTextDocument(a,{preview:false});await vscode.commands.executeCommand('type',{text:'human '});const dispatched=calls.filter(row=>row.method==='editor/dispatch').length;await vscode.commands.executeCommand('vcp.applyEditorChanges');assert.equal(calls.filter(row=>row.method==='editor/dispatch').length,dispatched);assert(a.getText().includes('human '));coverage.stalePreview=true;errors.length=0;await vscode.commands.executeCommand('workbench.action.files.revert');
