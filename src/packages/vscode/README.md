...
Build an unsigned installable candidate with `npm run build`, then `node scripts/package.cjs --engine <absolute-vcp.exe> --engine-manifest <native-distribution-result.json>`. The engine hash must match the native candidate record. Output is `artifacts/p4-vscode-package`, containing the VSIX and its external inventory/provenance manifest. Packaging uses pinned official VSCE, independently verifies archive bytes, and does not publish anything. `npm run package -- ...` combines both steps (use `npm.cmd` from PowerShell to preserve argument forwarding).
