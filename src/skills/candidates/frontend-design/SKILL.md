...
Use this workflow for a requested interface or interaction change. A non-UI bug does not need a design exercise. Establish the user's task, the relevant screen and its success state from the brief and existing project. Ask only for missing information that changes the result. For a new interface without an established design system, write a short brief first: audience, primary task, content priority, tone and constraints. Derive a small token set for type scale, spacing and color roles from that brief instead of a default template.
