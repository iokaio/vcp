...
This package bundles no provider or SDK reference. Establish exact request, response and streaming behavior from the project's installed SDK source and the provider documents the project supplies. Record exact versions, document dates and unresolved claims. Read only references relevant to the selected provider and task; do not copy model/price tables into permanent guidance or switch providers when documentation is unavailable. Continue bounded source work and report the specific compatibility check not run.
