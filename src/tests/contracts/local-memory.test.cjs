...
const corpus = { documents: [
  { id: 'current', workspace: 'a', current: true }, { id: 'old', workspace: 'a', current: false }, { id: 'foreign', workspace: 'b', current: true }
], queries: [{ id: 'query', workspace: 'a', lexical_required: ['current'], semantic_required: ['current'] }] };
const query = () => ({ status: 'pass', phase: 'query', queries: [{ case: 'query', workspace: 'a', lexical: ['current'], semantic: ['current'], ann_recall_at_3: 1, reopen_ms: 1, inference_us: 1, query_us: 1 }] });
...
test('build evidence binds corpus, model and both real search engines', () => {
  const good = { status: 'pass', phase: 'build', documents: 3, corpus_sha256: 'a'.repeat(64), model_spec_sha256: 'b'.repeat(64), dimensions: 384, metric: 'cosine', vector_engine: 'diskann', lexical_engine: 'tantivy', receipt_sha256: 'c'.repeat(64) };
