...
function verifyReference(actual, reference, lockSha256) {
  const features = actual.policy === 'munarium-local-libraries-v1' ? ['munarium-datastore/vector-diskann'] : [];
  if (reference.schema_version !== 1 || reference.policy !== actual.policy ||
      !['munarium-local-libraries-v1', 'local-cpu-embedding-v1'].includes(actual.policy) ||
...
module.exports = { munariumClosure, embeddingClosure, verifyReference };
