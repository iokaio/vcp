...
calls! {
    ...
    BackupStatus(crate::backup_publisher::StatusRequest) => "backup/status",
    BackupCreate(crate::backup_publisher::Create) => "backup/create",
    BackupRead(crate::backup_publisher::Read) => "backup/read",
    BackupRetry(crate::backup_publisher::Retry) => "backup/retry",
    BackupCancel(crate::backup_publisher::Cancel) => "backup/cancel",
...
impl Call {
    ...
    pub fn mutation(&self) -> Option<&Mutation> {
        match self {
            ...
            Self::BackupCreate(p) => Some(&p.mutation),
            Self::BackupRetry(p) => Some(&p.mutation),
            Self::BackupCancel(p) => Some(&p.mutation),
    ...
    pub fn validate(&self) -> Result<(), &'static str> {
        ...
        match self {
            ...
            Self::BackupStatus(p) => p.validate(),
            Self::BackupCreate(p) => p.validate(),
            Self::BackupRead(p) => p.validate(),
            Self::BackupRetry(p) => p.validate(),
            Self::BackupCancel(p) => p.validate(),
...
pub enum ResultValue {
    ...
    BackupStatus(crate::backup_publisher::StatusView),
    BackupJob(crate::backup_publisher::JobView),
