...
pub fn definition(name: &str) -> Result<Value> {
    let (description,properties,required)=match name {
        "vcp_read"=>("Read one workspace-relative UTF-8 file path, not an artifact ID. Include all four arguments; use start_line:null,end_line:null for a whole-file read (max_bytes at most 1048576). Otherwise line bounds are 1-based and inclusive. Ranges expose a byte-bounded part of a source up to 64 MiB, with full source version and next_line continuation; complete means the entire file was returned.",json!({"path":{"type":"string"},"max_bytes":{"type":"integer"},"start_line":{"type":["integer","null"]},"end_line":{"type":["integer","null"]}}),vec!["path","max_bytes","start_line","end_line"]),
