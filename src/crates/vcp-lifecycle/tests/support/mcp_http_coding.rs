    ...
    for backend in [BackendKind::Sqlite, BackendKind::Files] {
        ...
        host.configure_mcp_remote(
            RemoteRegistration::fixture_roots(
                ...
                .unwrap(),
                BTreeSet::from(["write_marker".into()]),
                ...
                let (action, tool, digest, arguments) = match index {
                    ...
                    2 => {
                        ...
                        *selected.lock().unwrap() = Some(digest.clone());
                        ("call", "write_marker", digest, "{}".into())
                    },
                    3 => ("call", "write_marker", selected.lock().unwrap().clone().unwrap(), "{}".into()),
                    _ => {
        ...
        assert_eq!(std::fs::read_to_string(peer.marker()).unwrap(), "effect\n");
