...
async fn run(backend: BackendKind, oversized: bool) {
    ...
    for phase in 0..3 {
        ...
        endpoint["data"]["endpoints"][0]["max_prompt_tokens"] = serde_json::json!(26_142);
        ...
        host.configure_continuity(
            ...
            vcp_context::compaction::Config {
                keep_recent_pairs: 1,
                // The 26,142 prompt ceiling minus the 512 safety reserve leaves
                // 25,630 input bytes. A larger historical preview must still fail
