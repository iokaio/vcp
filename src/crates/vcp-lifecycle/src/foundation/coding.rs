...
pub fn schemas() -> Value {
    ...
    schemas.as_array_mut().unwrap().push(json!({
        ...
        "description":"Run the owner's configured acceptance checks against current sources. For unchanged analysis, citations must contain at least one relevant complete same-task artifact ID, such as the top-level evidence UUID from a successful vcp_read, vcp_list or vcp_search result. Use artifact IDs, not paths or effect IDs. Resolve verification.outstanding_issues and rerun before the final answer. complete:false means this tool records evidence without finalizing the task; the host decides completion.",
