...
impl CanonicalHost {
    ...
    pub fn control_envelope(
        ...
        self.worker.run_cleanup(move |context| {
            let current: Task = context.engine.store().state()
                .record(Collection::Task, task.as_str(), &context.config.workspace)?.decode()?;
            Ok(CommandEnvelope {
                version: 1, id, workspace: current.scope.workspace,
                session: current.scope.session, task: Some(task),
                caller: context.config.actor.clone(),
                controller: context.engine.controller().clone(),
                owner_epoch: context.engine.owner_epoch(), expected,
                steering: current.steering, payload,
            })
        })
    }
    ...
    pub fn stop(&self, command: CommandEnvelope) -> Result<CommandReceipt, String> {
        ...
        self.worker.run_cleanup(move |context| {
            ...
            if command.workspace != context.access.workspace
                ...
                || command.caller != context.access.actor || !context.access.write {
                return Err("control scope or caller denied".into());
            }
            let (next, reason) = match &command.payload {
                Command::Transition { next, reason, verification: None }
            ...
            if let Some(receipt) = context.engine.store().state()
                .command(&command.workspace, &command.id, &command.digest()?)? {
                return Ok(receipt);
            }
            if !context.owner_alive || command.controller != *context.engine.controller()
            ...
            let current: Task = context.engine.store().state()
                .record(Collection::Task, task.as_str(), &command.workspace)?.decode()?;
            let scope = Scope { workspace: command.workspace.clone(), session: command.session.clone(), task: task.clone() };
            ...
            if current.state == TaskState::Paused && next == TaskState::Paused {
                if current.scope != scope || current.revision != command.expected
                    || current.steering != command.steering || reason.trim().is_empty()
                    || reason.len() > 4096 {
                    return Err("stale or invalid repeated pause".into());
                }
            } else {
                current.transition(&scope, command.expected, command.steering, next,
                    EventId::new(), reason.clone(), None, None)?;
            }
            let threads: Vec<_> = bindings.lock().map_err(|_| "binding lock poisoned")?
                .iter().filter(|(_, binding)| binding.scope == scope).map(|(id, _)| *id).collect();
            if threads.is_empty() && current.state == TaskState::Running {
                return Err("running task has no reachable retained owner".into());
            }
            for thread in threads {
                let view = runtime.inspect(thread).map_err(|e| format!("control owner: {e:?}"))?;
                if !view.local_hold {
                    // HoldWaiter cancellation does not cancel owned draining.
                    drop(runtime.hold(thread, &view.revision).map_err(|e| format!("control stop: {e:?}"))?);
                }
            }
            Ok(context.runtime.block_on(context.engine.handle(command, &context.access,
