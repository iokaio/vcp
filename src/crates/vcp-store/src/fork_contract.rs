...
pub(crate) fn marked(transaction: &Transaction) -> bool {
    transaction.events.iter().any(|event| {
        event.data["document_type"]
            .as_str()
            .is_some_and(|value| value.starts_with("vcp-session-fork"))
    })
}
            ...
            event.event.id == turn.cause
                ...
                && event.event.data["facts"].as_array().is_some_and(|facts| {
                    let matching: Vec<_> = facts
                        .iter()
                        .filter(|fact| {
                            fact["collection"] == "turn"
                                && fact["id"] == accepted.through_turn.as_str()
                        })
                        .collect();
                    matching.len() == 1
                        && matching[0]["revision"]
                            == serde_json::to_value(turn.revision).unwrap_or_default()
                        && serde_json::from_value::<Turn>(matching[0]["value"].clone())
                            .ok()
                            .as_ref()
                            == Some(&turn)
                })
