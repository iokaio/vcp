...
Original VCP guidance, version 1.0.2. Create or revise a VCP skill package when a
...
Before native verification, reread every created or changed package file in full
and compute hashes from those current bytes. Cite only the smallest set of current,
complete, same-task read artifacts required by the configured checker; do not pass
superseded reads, search hits, patch effects or unrelated list artifacts merely
because they are available. Invoke the configured verifier once. If it reports a
missing or conflicting store record, retain the package bytes, report native
verification as not run and stop; do not retry with guessed or stale identifiers.
