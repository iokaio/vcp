...
**Reuse sequence.** First build the selected Codex engine/CLI on native Windows and retain its working task, terminal and execution structure. Locate model, persistence, context and tool-dispatch boundaries; insert adapters for VCP budgets/routing and local memory. Exercise C02/C03/C04 behind those boundaries before deciding which components need extraction. C01's internal lifecycle remains useful now; public protocol exports wait. This sequence is future P0 work, not a completed fork or import.
...
Review upstream fixes before each release and on relevant security/compatibility reports; do not auto-update vendored code at runtime. Support rollback through the existing data-format rules. Track patch size, upstream lag, build cost, and time to import a representative fix so selective reuse can be compared with a fork using evidence. P8-06 includes an update rehearsal and checks the packaged notices/source manifest.
...
| ADR-013 | Upstream reuse/license | Maximum reasonable Codex reuse, selected Gemini/Munarium code; Apache-2.0 distribution confirmed | P0-07/08/09, P8-06 |
