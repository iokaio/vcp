...
| `turn/start` | Selected workspace/session/new root and caller turn; C | Exact pinned profile budget, unused IDs, zero revisions, native fingerprint and current controller; durable | Atomic Pending/Queued acceptance then one-use owned construction, activation and submission; retained receipt includes original turn identity | Configured local execution supports this adapter ([ADR-047](../adr/047-durable-public-run-start.md)); original root request ceiling/deadline survives restart, replay never constructs or submits |
...
| `diff/read` | Workspace/session/task/tool-effect identity; O | Current scope, retained initial proposal and both source captures, bounded range; read | Implemented by `Engine<Store>::public_diff` through the live host; `vcp-public-diff/1` proposed bytes | [ADR-049](../adr/049-retained-public-diff-evidence.md); historical proposals without typed capture are unavailable; no arbitrary filesystem read |
...
| `memory/query` | Exact workspace/session/task scope; O | Current access at capture and finish; requires `memory/query-sources/1`; host limit 4096 bytes/64 results | Implemented by live `PublicConnection::memory_query`; tagged artifact or claim sources, watermarks, degradation and truncation | [ADR-051](../adr/051-authenticated-public-memory-query.md); shared lexical runner, no hidden indexing/model call |
...
| `editor/context` | Workspace/task/connection-owned documents; C | Current task/steering revision, native root/disk validation; durable metadata only | Bounded observations and exact current `closed` retirement; superseded undispatched proposals cancelled, no autoresume | Implemented under `editor/prepared-edits/1`; P4-03 native qualification passed on Files and SQLite |
| `editor/prepare` | Workspace/task/current observations; C | Running execution binding, policy and task revision; durable immutable per-file effects | Change ID equals original command ID; text stays connection-local unless explicitly captured | Current controller and actual task authority required; approval does not resume |
| `editor/changeRead` | Workspace/task/change; O | Access and retention checked; read-only | Canonical bounded per-file metadata/outcomes, without transient replacement text | Unknown outcomes are historical evidence, not replay authority |
| `editor/dispatch` | Workspace/task/change/file; C | Change revision, current observation/native evidence and fresh policy admission | Durable per-file dispatch/execution before `apply: true`; replay always `apply: false` | No cross-file atomicity, automatic saving or automatic rollback |
| `editor/changeResult` | Workspace/task/change/file/execution; C | Current change revision and actual document/native evidence | Applied/rejected/unknown buffer receipt; disk evidence remains separate | A client success assertion alone is not verified effect evidence; P4-03 native qualification passed on Files and SQLite |
