...
The current recorded package is an unsigned debug qualification candidate:
...
| Acceptance | Observed proof | Remaining gap | Next action |
|---|---|---|---|
| Full ordinary bytes versus prompt tails, child history, provenance and explicit compaction/purge gaps | Final packaged oversized-output case verifies full paged bytes/hash, scoped event links, compaction preservation and purge/no-replay on both stores; separate child transcript/recovery and structured finding cases pass. | The complete selected-refinement history matrix, especially packaged MCP identity/content across compaction and reopen, is not established by those separate cases. The fresh native MCP conversation receipt passes, but does not by itself qualify packaged compaction/reopen. | Retain the passing native receipt; map remaining packaged MCP/refinement provenance conditions separately rather than inferring them from ordinary process output. |
| Exclude typed credentials/recovery keys before serialization across request, environment, artifacts, terminal/JSONL, inspectors, diagnostics and export | Packaged preflight returns safe errors before acceptance/network/provider work; capture exclusion annotations are explicitly asserted; key-exclusion and independent exported snapshot checks pass. | No single complete sensitive-marker audit across every named surface is documented. Correct policy annotations alone do not prove all secret boundaries. | Trace synthetic typed markers through each required surface and attach existing evidence first; keep any unobserved surface open. |
